# AI 资产生产流程与 Prompt 规范

项目阶段：阶段 11，表现和可读性打磨  
资产目标：用 AI 生成工具快速产出可替换的游戏视觉资源，并通过统一命名、色彩规范和可读性验收，形成可复用的资产生产流程。
...
- 把模糊视觉需求拆成结构化 prompt。
- 围绕产品体验定义视觉约束，而不是只追求“好看”。
- 把 AI 生成结果纳入资源命名、导入、替换和验收流程。
- 用可读性标准校验生成结果是否服务于用户理解。
...
- 视觉边界要接近 Godot 碰撞边界，避免玩家误判能否站立或通过。
- 颜色不能是唯一信息来源，关键对象还要有形状、纹理或图标差异。
- 角色状态必须可识别：普通、Blocker、Parachute、Climber 不能混淆。
- UI 图标必须在小尺寸下可读，能表达当前能力和剩余次数。
- 动画或特效不能遮挡缺口、桥面、弱墙、出口或可点击角色。

## 2. 资产目录

| 目录 | 用途 |
| --- | --- |
| `assets/sprites/` | 角色基础状态、行走帧、Blocker、Climber、Parachute |
| `assets/tiles/` | 普通地面、实体墙、弱墙、桥梁、缺口相关视觉 |
| `assets/props/` | 出生口、出口、救援反馈、弱墙裂纹 |
| `assets/ui/` | 五种能力图标、按钮状态、关卡完成标记 |
| `assets/audio/` | 救援、死亡、能力使用、胜利、失败等短反馈 |

以下共 31 组 prompt，对应当前项目的第一批 AI 资产生产清单。
